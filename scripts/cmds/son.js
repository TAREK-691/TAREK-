...
module.exports.onStart = async function({ api, event }) {
    ...
    if (mentionIDs.length === 0) {
        return api.sendMessage("❌ দয়া করে একজনকে মেনশন করো!", event.threadID);
    }
    ...
    if (subCommand === "off") {
        if (userResponses[mention] && userResponses[mention].active) {
            ...
            return api.sendMessage(`✅ ${event.mentions[mention]} এর জন্য গালি বন্ধ করা হয়েছে।`, event.threadID);
        } else {
